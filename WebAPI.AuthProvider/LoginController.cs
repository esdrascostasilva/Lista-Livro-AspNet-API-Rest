    ...
    public class LoginController : ControllerBase
    {
        ...
        public async Task<IActionResult> Token(LoginModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    //creat token - header + payload (claim, direitos) + signature
                    var direitos = new[]
                    {
                    ...
                    var chave = new SymmetricSecurityKey(System.Text.Encoding.UTF8.GetBytes("alura-webapp-authentication-valid"));
